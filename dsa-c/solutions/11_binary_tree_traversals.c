...
// Teaches: recursive tree traversal, struct with pointers to self, collecting results in array
#include <stdio.h>
#include <stdlib.h>
#include <assert.h>
...
void ia_push(IntArray* a, int val) {
    if (a->length == a->capacity) {
        a->capacity *= 2;
        a->items = realloc(a->items, sizeof(int) * a->capacity);
    }
