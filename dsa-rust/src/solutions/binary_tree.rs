...
/// A binary tree node. Children are Option<Box<...>> — the standard
/// way to represent optional heap-allocated children in Rust.
...
fn pre_order_walk<T: Clone>(node: &Option<Box<BinaryNode<T>>>, result: &mut Vec<T>) {
    if let Some(n) = node {
        result.push(n.value.clone()); // visit root
        pre_order_walk(&n.left, result); // then left
        pre_order_walk(&n.right, result); // then right
    }
}
...
fn in_order_walk<T: Clone>(node: &Option<Box<BinaryNode<T>>>, result: &mut Vec<T>) {
    if let Some(n) = node {
        in_order_walk(&n.left, result);   // left first
        result.push(n.value.clone());     // then root
        in_order_walk(&n.right, result);  // then right
    }
}
...
fn post_order_walk<T: Clone>(node: &Option<Box<BinaryNode<T>>>, result: &mut Vec<T>) {
    if let Some(n) = node {
        post_order_walk(&n.left, result);  // left first
        post_order_walk(&n.right, result); // then right
        result.push(n.value.clone());      // root last
    }
}
